...
class TextFineTuning:
    def __init__(self, path, save, model_type, classes=None, epochs=3, batch_size=8):
        self.path = path
        self.save = save
    ...
    def encode_tags(self, tags, encodings, tag2id):
    ...
    def use(self):
        global train_dataset, model, val_dataset
        ...
        if self.model_type == "token_classification":
            ...
            train_labels = self.encode_tags(train_tags, train_encodings, tag2id)
        ...
        if self.model_type == "q+a":
            train_contexts, train_questions, train_answers = self.read_squad('squad/train-v2.0.json')
            val_contexts, val_questions, val_answers = self.read_squad('squad/dev-v2.0.json')
        ...
        model.fit(train_dataset.shuffle(1000).batch(self.batch_size), validation_data=val_dataset, epochs=self.epochs,
